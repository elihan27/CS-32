...
bool compareStudentPtr(const Student* lhs, const Student* rhs)
{
    ...
    // DONE: Using the same criteria as compareStudent,
    //       compare two Students POINTED TO by lhs and rhs.  Do it in one line
...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (int unsorted = 2; unsorted< s.size(); unsorted++)
    {
        Student nextItem = s[unsorted-1];
        int loc = unsorted-2;
        while ((loc >= 0) && comp(nextItem, s[loc]))
        {
            s[loc+1] = s[loc];
            s.pop_back();
            --loc;
        }
        s[loc+1] = nextItem; // Insert nextItem into sorted region
    ...
    //       parameter comp.See the errata page entry for page 313 at
...
int main()
{
    ...
    if (TEST_INSERTION_SORT)
    {
        ...
        assert(isSorted(students)); //problemo is RIGHT HERE
    ...
    // destruction time.
    {
        ...
        vector<Student*> vPointers;
        for (int i= 0; i != auxStudents.size(); i++)
        {
            vPointers.push_back(&auxStudents[i]);
        }
        
        
        sort(vPointers.begin(), vPointers.end(), compareStudentPtr);
        
        
        
        for (int i= 0; i != auxStudents.size(); i++)
        {
            students[i] = *vPointers[i];
